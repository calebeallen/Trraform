...
<div on:click={click} transition:fade={{duration: 150}} class="fixed top-0 left-0 grid w-full h-full px-2 overflow-auto bg-black bg-opacity-50 place-items-center">
    <div bind:this={contentContainer} on:clickOutside={() => dispatch("close")} transition:fly={{duration: 150, y: 50}} class="w-full p-4 bg-zinc-800 outline outline-1 outline-zinc-700 rounded-2xl shadow-2xl shadow-black {addedClasses}">
